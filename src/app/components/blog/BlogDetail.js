...
import { getHomePage1, getHomePageDetail, getHomePage } from '../../_redux/itemSlice';
...
const BlogDetail = ({ Id }) => {
  ...
  const extractHtmlContentBetweenHeadings = (htmlContent) => {
    ...
    if (isContent) {
      ...
      const headings1 = doc.querySelectorAll('p');
      ...
      headings1.forEach((heading) => {
        ...
        if (x == 0) {
          ...
          newArray1.push(content);

          setStringArray1(newArray1);
          x++;
        }
  ...
  const [stringArray1, setStringArray1] = useState([]);
  ...
  let x = useSelector(getHomePageDetail);


  const [faq, setfaq] = useState([]);


  const bloglist = x ? x.blogs : [];
  const stafflist = x ? x.expert : [];
  const z = x ? x.blogfaqs : [];
  const b = x ? x.blogauthor : [];
  ...
  useEffect(() => {
    if (!(bloglist === undefined)) {
      if (bloglist.length > 0) {
        extractHtmlContentBetweenHeadings(bloglist);
        MyData(bloglist);
  ...
  const MyData = async (bloglist) => {
    bloglist.forEach((v) => {
      if (v.urlname == Id.split('&')[0]) {
        z.forEach((a) => {
          if (a.blogid == v.id) {

            let i = [a];
            console.log(i)
            if (a.question.length > 0 || a.answer.length > 0) {
              setfaq(i);
  ...
  return <div className='pl-8 pr-8 pt-2'>
                ...
                <div className={` lg:col-span-3 tablet:col-span-4 mobile:col-span-4 mobile-small:col-span-4 mobile-extra-small:col-span-4`} key={k} >
                  ...
                  <div className={`grid grid-cols-1`}>
                    <div>
                      <div className="my-3 mb-1  text-dark " style={{ fontSize: '14px' }}>
                        <button className='d-none d-lg-inline-block' onClick={(e) => navigate.push('/')} style={{ color: 'rgb(247, 93, 52)' }}>Home</button>

                        <span className='ml-1 d-none d-lg-inline-block'>❯</span>
                        &nbsp;
                        <button onClick={(e) => navigate.back('')} style={{ color: 'rgb(247, 93, 52)' }}>Blog</button>
                        &nbsp;
                          ...
                          <div key={i} className={`grid grid-cols-8 mobile-extra-small:gap-16`}>
                            ...
                            <div className={`mobile-extra-small:col-span-6 lg:col-span-3 lg:-ml-12 tablet:col-span-6 mobile-small:col-span-6 mobile:col-span-6 `}>
                              <div className="grid grid-cols-1 ml-0">
                                <div>
                                  <Link href={`/Author/${en.name}`}>
                                    <h6 className="authorname h1color font-bold">{en.name}</h6>
                                  </Link>
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-1 ml-0">
                                <div>
                                  <Link href={`/Author/${en.name}`}>
                                    <h6 className=" h2color">{en.designation}</h6>
                                  </Link>
                                </div>
                              </div>
                              ...
                              <div className="grid grid-cols-1 ">
                                <div >
                                  <a style={{ color: '#316FF6' }} target="_blank" href={en.fb}>
                                    <i className="fa-brands fa-facebook"></i>
                                  </a>
                                  <a style={{ color: '#0077B5' }} target="_blank" href={en.linkedin}>
                  ...
                  <div className={`grid grid-cols-1`}>
                    <div className='d-flex text-dark  flex-wrap align-items-center justify-content-between'>
                      <div className="px-3 py-2 rounded mt-2 mb-3" style={{ backgroundColor: 'aliceblue', width: 'max-content', maxWidth: '100%', border: '1px solid rgb(170, 170, 170)' }}>
                        <h6 className="mb-0 d-flex justify-content-between align-items-center fw-bold">Table of Contents <span className="ms-3 cursor-pointer">
